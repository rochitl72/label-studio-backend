...
def _load_org_config_overrides() -> dict:
    """Optional deployment-time config files, in the org's own format.

    On the physical server, the deployment team drops values into two files
    at the backend repo root — sitting next to this package, not inside it:

      db_config.py   DB_CONFIG = {"dbname", "user", "password", "host", "port"}
                      Same shape used by the org's other deployed services
                      (e.g. tcga-backend) as the single source of truth for
                      the live app's database connection.

      config.py      APP_CONFIG = {...} — everything else: SECRET_KEY, the
                      bootstrap admin, storage/export paths, upload limits.

    Both are placeholder templates in git (see backend/db_config.py and
    backend/config.py) and the deploy workflow excludes both from every sync,
    so real values entered on the server persist across updates.

    Neither file is required. Local development, the Docker Compose
    deployment, and the `.env` file all keep working exactly as before when
    they're absent — this only adds a second, optional input, and it takes
    priority over `.env`/environment variables when present.
    """
    overrides: dict = {}
    ...
    try:
        from config import APP_CONFIG  # type: ignore[import-not-found]
    except ImportError:
        pass
    else:
        # Only forward keys Settings actually defines, so a typo'd or
        # future key in APP_CONFIG fails loudly instead of being silently
        # ignored.
        valid_fields = set(Settings.model_fields)
        unknown = set(APP_CONFIG) - valid_fields
        if unknown:
            raise RuntimeError(
                f"config.py's APP_CONFIG has unknown setting(s): {sorted(unknown)}. "
                f"Valid settings: {sorted(valid_fields)}"
            )
        overrides.update(APP_CONFIG)

    return overrides


settings = Settings(**_load_org_config_overrides())
